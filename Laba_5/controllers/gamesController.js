...
// CREATE - Додати нову гру (з транзакцією)
...
// UPDATE - Оновити гру (з транзакцією)
...
// БІЗНЕС-ОПЕРАЦІЯ: Створення турніру з декількома іграми
...
async function getTeamStats(req, res) {
  ...
  try {
    ...
    const homeGames = await Game.findAll({
      where: { team1_id: teamId },
      include: [{ model: Team, as: 'team2' }]
    });

    const awayGames = await Game.findAll({
      where: { team2_id: teamId },
      include: [{ model: Team, as: 'team1' }]
    });
    ...
    // Підрахунок статистики для домашніх ігор
    homeGames.forEach(game => {
      stats.goalsFor += game.score_team1;
      stats.goalsAgainst += game.score_team2;
      
      if (game.score_team1 > game.score_team2) stats.wins++;
      else if (game.score_team1 === game.score_team2) stats.draws++;
      else stats.losses++;
    });

    // Підрахунок статистики для гостьових ігор
    awayGames.forEach(game => {
      stats.goalsFor += game.score_team2;
      stats.goalsAgainst += game.score_team1;
      
      if (game.score_team2 > game.score_team1) stats.wins++;
      else if (game.score_team2 === game.score_team1) stats.draws++;
      else stats.losses++;
    });
